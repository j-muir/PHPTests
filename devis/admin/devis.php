...
$arrayCommandes = getCommandes($connexion, '0', $exerciceencours);

$idCommandeFrom = 0;
if (isset($_GET['q'])) {
    $idCommandeFrom = $_GET['q'];
}
...
<head>
    <base href="../">
    <meta charset="utf-8"/>
    <title>VDMA | Gestion des devis</title>
                        ...
                        <div class="card card-custom">
                            <div class="card-header flex-wrap">
                                <div class="card-title">
                                    <h3 class="card-label">Gestion des devis</h3>
                                </div>
                                <div class="card-toolbar">
                                    <!--begin::Button-->
                                    <input type="hidden" id="idCommandeFrom" value="<?php echo $idCommandeFrom; ?>"/>
                                    <button style="float:right;" id="addCommandes" type="button"
                                            class="btn btn-primary ml-3" data-keyboard="false" data-toggle="modal"
                                            data-target="#staticBackdropCommande">Ajouter un devis
                                    </button>
                                    <!--end::Button-->
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
                                    ...
                                    <tr>
                                        ...
                                        <th style="width:8%;">Date/heure visite</th>
                                        <th>Objet</th>
                                        <th style="width:10%;">Num. Tél.</th>
                                    ...
                                    foreach ($arrayCommandes as $keyA => $valA) {
                                        $client = $valA['client']['nom'] . ' ' . $valA['client']['prenom'];
                                        if ($valA['client']['societe'] != "") {
                                            $client = $valA['client']['societe'];
                                        }

                                        if (!empty($valA['adressetravaux'])) {
                                            $cp = $valA['client']['codepostal'];
                                        } else {
                                            $cp = $valA['codepostal'];
                                        }

                                        $lesnumtel = '';
                                        if (!empty($valA['tel']) && !empty($valA['telportable']) && $valA['telportable'] !== ' ') {
                                            $lesnumtel = $valA['telportable'] . '<br>' . $valA['tel'];
                                        } else if (!empty($valA['telportable']) && $valA['telportable'] !== ' ') {
                                            $lesnumtel = $valA['telportable'];
                                        } else {
                                            $lesnumtel = $valA['tel'];
                                        }

                                        echo '<tr class="modCommande" data-id="' . $keyA . '">
                                            <td><b>' . $client . '</b></td>                                        
                                            <td data-sort="' . $valA['datetri'] . '" style="width:10%;">' . $valA['date'] . '</td>    
                                            <td>' . $valA['travauxaexecuter'] . '</td>      
                                            <td style="width:10%;">' . $lesnumtel . '</td>    
                                            <td>' . $cp . '</td>
                                            <td>' . $valA['etat'] . '</td>
                                            <td data-sort="' . $valA['montanthtMRtri'] . '" style="width:10%;">' . $valA['montanthtMR'] . '</td>
                                        </tr>';
                                    }
...
<script src="assets/js/pages/commandes/main.js"></script>
